...
- **β↔memory convention (R2, corrected 2026-07-07):** the code uses the standard EMA `Lbar ← β·new + (1−β)·old`, so **β is the weight on the NEW observation ⇒ HIGH β = SHORT memory (tracks drift fast), LOW β = LONG memory (ignores drift)**. Effective memory length ≈ (1−β)/β epochs: β=0.8 → ~0.25 ep (very short), β=0.5 → ~1 ep, β=0.3 → ~2.3 ep. (Earlier drafts labelled β=0.8 "long memory" — that was inverted; fixed here so the ablation is read correctly.)
- **β is a FIRST-CLASS ABLATION {0.3, 0.5, 0.8}** (the cheap validator Proposition 3 / method-note-v2 demands, NOT a hardcode): report the β\* that maximizes φ’s 1-epoch-ahead hit-rate + the full hit-rate-vs-β curve. Pre-registered reading under the corrected convention: if the **state overlay (drift) is real**, the shorter-memory arms (higher β, which track migration) should predict next-epoch binding better, while the **long-memory arm β=0.3** (nearest to a fixed lifelong label — the "ignore-drift" arm) should LOSE. A mid β\* ≈ 0.5 winning = "predictor memory ≈ the measured short trait timescale" (Gate-α ~5-epoch trait *with* drift) — a mechanism link (trait horizon ⇒ predictor memory). NOTE none of {0.3,0.5,0.8} reaches a literal 5-epoch memory ((1−β)/β=5 ⇒ β≈0.17); if β* pins at the low end (0.3) add a longer-memory arm (β∈{0.15,0.2}) before claiming the horizon match.
