...
- **CODE REVIEW (2026-07-07) — 3 defects, fixes gated on `card-pb-fixes` (R6 commit b73d078):** **R1 (BLOCKER)** the v1 safety floor is INERT (adaptive raise feeds only the FLOPs counter + log, never the attack — floor_attacks fixed at 3 steps) → the "floor self-corrected" smoke claim is RETRACTED, reported FLOPs over-count after any raise, and **v2's l1-floor sweep is a no-op until fixed**. **R3** φ's Lbar decays from floored (under-attacked) losses → self-reinforcing misprediction (plausibly the real F9 driver). **R2** card β↔memory labels inverted vs code EMA (high β = short memory). Fix order at gate: R1→R3→R2→re-smoke (the REAL floor validation). Reframed F9: mispredicted l1 floored at a FIXED 3/20 steps → collapse, entrenched by the φ update.
- **CARD-PB RAN + s0 KILLED (F9, 2026-07-06):** paper-grade @8/255 s0 = union 40.6 (−1.1pp vs reactive 41.7), l1 −3.2pp, at FLOPs 0.522 (over-counted, see R1). FLOPs cut real, robustness cost real (l1-specific). Pre-registered kill fired → predictive halted at s0, not escalated. **Next lever = CARD-PB v2 (Kiet-DECIDED 2026-07-07, `docs/CARD-PB_v2_CONFIDENCE_FLOOR.md`, DRAFT — launch gated):** predictability-aware floor `floor_g=clamp(k_min+round(k_span·miss_g),…)`, miss_g=1−recall_g measured unbiased on the recal subset (NOT the probe — the v1 probe-driven guard is blind to l1 because the l1 probe is the weakest attack, F1: it raised l2 3→5 but never l1). **Option B PRIMARY**, sweep k_span∈{8,12,16} each mapped to measured FLOPs; A (fixed l1 floor 8) = fallback/ablation. Pre-reg winning arm: union within ±0.3pp of reactive AND l1 drop <2pp AND FLOPs≤0.60; if no arm → FINDING "l1 predictability floors efficiency". Needs new code (per-norm miss + `phi/miss_rate_{norm}` keys) + `configs/cardpb_v2.yaml`. **Launch gate: reactive 3-seed baseline (~06:30 UTC 2026-07-07) → write code+config → show card+sweep → Kiet GO.** Do NOT relaunch predictive unattended. v1 artifact `results/reactive_vs_predictive_s0.md`.
- **⚠ RECIPE RESET (Kiet, 2026-07-07) — single-recipe (RAMP) plan; ALL our-recipe numbers ARCHIVED.** Method lane moves to RAMP's recipe (lr 0.05→0.005 ×10 drop at ep70, 80ep, save every 10, train==eval@8/255) + training inner-max **10/10/10** (was 10/10/20) for a confound-free comparison with RAMP at every epoch. **"reactive baseline 41.90" RETRACTED as active** (archived `results/archive/our_recipe_50ep_drop25/`); new baseline = RAMP-recipe reactive rerun. Plan in `docs/EXPERIMENT_TABLE.md`: PHASE 1 develop @ep50 1-seed (reactive→predictive v2→β; Colab avg/3a) → PHASE 2 winner to ep80 3-seed + full-AA. KEEP RAMP repro ep40/50/80 (43.6/42.9/46.1). FLAGS: trainer save_freq+resume, λ reconcile (5 vs 2). Reactive training budget now 30 steps (FLOPs denom). **Launch nothing — awaiting GO.**
- ~~**Reactive (soft T=0.25) @8/255 — 3-SEED LOCKED:** union 41.90±0.22, l1 48.10±0.43~~ **ARCHIVED (our-recipe 50ep, retired). Do not cite as active.**
...
## Last-updated stamp
- 2026-07-07 UTC (reset) by Claude Code: **RECIPE RESET** → single-recipe RAMP plan. Archived ALL our-recipe numbers (reactive 41.90 3-seed, predictive v1/v2) to `results/archive/our_recipe_50ep_drop25/`; 41.90 baseline RETRACTED as active. Training attack → 10/10/10 (RAMP-matched). New plan `docs/EXPERIMENT_TABLE.md` (PHASE1 ep50 1-seed → PHASE2 ep80 3-seed). RAMP repro (43.6/42.9/46.1) kept as reference; provenance validated (upstream be4971f, pristine, static=70+10; λ=5 vs paper 2). v2 fix (miss VOLUME + adaptive off) committed 1c061d5. Launch nothing — awaiting GO.
- 2026-07-07 UTC by Claude Code: CARD-PB code review — 3 defects (R1 inert floor=BLOCKER, R3 biased φ update, R2 β labels); R6 restore-point commit b73d078 on branch `card-pb-fixes`; fixes gated to reactive baseline (~06:10). Reactive s2 running (19/50 @ 03:50). v1 "floor validated" claim RETRACTED.
