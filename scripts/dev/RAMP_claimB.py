# GENERATED by scripts/dev/patch_ramp_claimb.py from external/RAMP/RAMP.py (uiuc-focal-lab/RAMP).
# Snapshot for reading only. Regenerate on the training machine; do NOT hand-edit. Claim-B injections marked '===CLAIM-B==='.
...
def parse_args():
    ...
    parser.add_argument('--cb_tau',   type=float, default=0.1)
    parser.add_argument('--pretraining', action='store_true')
    parser.add_argument('--max', action='store_true') # whether to use max strategy for L1 Linf perturb
    parser.add_argument('--bindaware_variant', type=str, default='none',
        choices=['none', 'augment', 'replace'],
        help='CARD-8b default-off binding-aware objective: none keeps RAMP; '
             'augment keeps hard L_max and adds aux*soft; replace uses soft only')
    parser.add_argument('--bindaware_t', type=float, default=0.25)
    parser.add_argument('--bindaware_aux', type=float, default=0.3)
    ...
    if args.bindaware_variant != 'none' and not args.max:
        raise ValueError('--bindaware_variant requires --max so the hard L_max term exists')
    return args
                    ...
                    if args.loss in ['ce']:
                        ...
                        if args.kl or args.mse or args.cosine:
                            ...
                            if args.max:
                                ...
                                if args.bindaware_variant == 'none':
                                    loss_best = loss_hard
                                else:
                                    ...
                                    if args.bindaware_variant == 'augment':
                                        ...
                                        raise ValueError('unknown bindaware_variant {}'.format(
                                            args.bindaware_variant))
                        ...
                        loss = loss_best + loss_kl * args.lbd + _rep
